...
class Sensors():
    ...
    def __init__(self,location: str,post_code: str,bucket: str,local_garbage_path: str='/tmp/garbage_bin',image_name:str='waste_image.jpg'):
        ...
        self._bucket=bucket
        self._local_image_full_path=get_image_full_path(local_garbage_path,image_name)
    ...
    def getUniqImageKey(self,waste_image_timestamp: float) -> None:
        ...
        destination_path = os.path.join(self._sensor_deployed_location,self._sensor_deployed_post_code, key)

        return destination_path
        
        
    def build_waste_weight_stats(self,weight:float,waste_image_timestamp: float) -> None:

        #weight=random.randint(1,10)
        uniq_key=f'waste_image_{waste_image_timestamp}.jpg'

        destination_path = os.path.join(self._sensor_deployed_location,self._sensor_deployed_post_code, uniq_key)
    ...
    def trigger_camera(self,shutter_speed: int,clip_duration: int) -> None:
        ...
        mkdir_cmd_result=subprocess.Popen(shlex.split(mkdir_cmd),stdout=subprocess.PIPE,stderr=subprocess.PIPE)
        done=False
        ...
        camera_cmd=subprocess.Popen(shlex.split(cmd),stdout=subprocess.PIPE,stderr=subprocess.PIPE)
