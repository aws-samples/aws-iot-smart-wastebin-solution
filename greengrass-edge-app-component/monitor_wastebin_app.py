...
def monitor_waste_bin():
    ...
    while True:
        try:
            ...
            print(f'current_weight={current_weight} - previous weight={previous_weight} - calculatred weight:{sensors.calculateThreshold(previous_weight)}')
            if current_weight >= sensors.calculateThreshold(previous_weight):
                #Take waste photo
                waste_image_timestamp=sensors.trigger_camera(sensors._shutter_speed_in_micro_secs,sensors._clip_duration_in_msec)
                event=sensors.build_waste_weight_stats(current_weight,waste_image_timestamp)
...
if __name__ == '__main__':
    try:
        ...
        cloud_bucket_name='demo-trash-bin'
        image_name='waste_image.jpg'
        location='Greenhill'
        post_code='HA11AA'

        sensors=Sensors(location,post_code,cloud_bucket_name,temp_local_image_path,image_name)
